...
description: Extending Designer-generated code — the whole `{BC}/Model/{Agg}/` tree is Generator-owned and hermetic (ForceOverwrite, never edit). The BC facade's Außentür exposes `{agg}(): {Agg}Read` (read-only queries + lists), `process()`, and an optional direct method per exposed rule-guarded Command; the write facade `{Agg}` (Commands + `event()`) is family-internal via the protected Kernel-Naht (`$this->handle()`). Developer surface: BC-level Processes under `{BC}/Process/{Name}/` (Generator-owned DTO/orchestrator/node stubs + `Event/`; Dev-owned `Query/`/`Repository/`/`Service/`) and the `{BC}/Closure/` catalog (own-BC-read-only Rule bodies, M9). Covers V1–V13 prohibitions, decision tree, generated-file modes, DomainResponse construction incl. `RuleViolation` 422. ClassVersion → `platform-versioning`. Workflow API → `platform-workflow`. Recipes → `platform-cookbook`.
zone: post-active
persona: C
prerequisites: []
...
└── <BC>/
    ...
    │                                                 — Rules-Layer G10)
    ├── Closure/                                   ← ★ DEVELOPER SURFACE — BC-level Rule catalog (sibling of Process/)
    ...
    └── Model/<Agg>/                              ← ★ GENERATED, NIE EDITIEREN (whole tree ForceOverwrite)
...
| **Lese-Fassade** `{Agg}Read` | `{Agg}/{Agg}Read.php` — `get{Agg}ById`/`ByIds` immer; `By{Key}`/`By{PluralKey}` nur mit öffentlichem Unique Key; `{agg}List` nur, wenn `Queries.json` sie deklariert. Inkl. `$version`, über `$bc->{agg}()` erreichbar. |
...
| **Rule** | `{BC}/Closure/{Name}.php` — ein endpunkt-gebundener, synchroner Ja/Nein-Wächter (`__invoke({Cmd}DTO): RuleResult`), DeveloperOwned, BC-scoped, wiederverwendbar über Aggregate hinweg. Rufbar ausschließlich via `$this->handle({Rule}::class)` (ClassVersion-fähig, `Closure/v{N}/`) — nie `new`. Liest Bestand nur aus dem **eigenen** BC (M9-Verbot: kein Fremd-BC-Import) — über die eigene Lese-Fassade für fassadengeführte Queries, oder direkt über die Kernel-Naht (`$this->context({Handler}::class, $payload)()`) für einen BC-internen Lesezugriff — die deklarierte interne Liste mit `limit: 1`, entschieden über `total` (s. Rule-Stub-Zeile unten). |
| **Guard** | `{BC}/Closure/Guard/Guard{Command}.php` — generierte, hermetische Chain-of-Responsibility-Closure je gebundenem Endpunkt: führt die konfigurierte Rule-Kette als UND mit Kurzschluss aus. Der generierte CommandHandler ruft nur `$this->handle(Guard{Command}::class)($cmd)` — keine Guard-Eigenlogik. |
...
| Aggregate Read Facade | `<BC>/Model/<Agg>/<Agg>Read.php` | Generator: ForceOverwrite | `extends <Domain>Context`. The aggregate's call surface for the Außentür, reached directly via `$bc->{agg}()`. Hosts the inline Query/list methods: `get<Agg>ById`/`ByIds` always, `By<Key>`/`By<PluralKey>` only on an aggregate WITH a public unique key, `<agg>List` only where `Queries.json` declares it. Each delegates `return $this->context(<Handler>::class, $dto, $version)();`. Dünne Fassade, keinerlei eigene Logik (Gremium B5). Fully regenerated — no merge, no `@flow-id`. |
...
| New aggregate command/query, or tighten an aggregate's invariants | Re-model the aggregate in the Aggregate Designer (`Aggregate.json`) and rebuild — the generated command/query/validator tree changes with it. A new **query** surfaces inline on the read facade `{Agg}Read.php` (`$bc->{agg}()`, the Außentür); a new **command** surfaces inline on the write facade `{Agg}.php`, reachable only family-internally (`$this->handle({Agg}::class)`) — model a Process around it to expose it externally via `$bc->process()`. There is no slot to hand-edit either facade. | Designer |
...
├── Closure/                                  ← ★ Rule catalog (sibling of Process/ and Model/)
...
└── Model/<Agg>/                             ← ★ GENERATED, NIE EDITIEREN (whole tree, no Platform/ segment)

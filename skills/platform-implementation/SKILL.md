...
└── <BC>/
    ...
    │   │                                            (own-BC read facade only, M9) ← your logic
    ...
    └── Aggregate/<Agg>/                          ← ★ GENERATED, NIE EDITIEREN (whole tree ForceOverwrite)
        ...
        ├── Query/{<Agg>ById,<Agg>ByIds,<Agg>By<Key>,<Agg>By<Custom>…,<Agg>ListFilter}.php  +  Handler/Get<Agg>{ById,ByIds,By<Key>,By<Custom>…,List}Handler.php
        ├── Event/{<Agg>{Created,Removed,Updated,…}.php, <Agg>EventRouter.php, <Agg>Events.php}
        ├── Repository/{<Agg>Repository,Query<Agg>{ById,ByIds,By<Key>,…},Transform<Agg>,Validate<Agg>,Persist<Agg>,Query<Agg>Additions,Query<Agg>List}.php
...
| **Lese-Fassade** `{Agg}Read` | `{Agg}/{Agg}Read.php` — Queries + Listen (`get{Agg}ById`/`ByIds`/`By{Key}` + `{agg}List`), inkl. `$version` (früherer Begriff: „Lese-Basis"). Über `$bc->{agg}()` erreichbar. |
| **Außentür** | Die öffentlichen Methoden der BC-Fassade: `{agg}(): {Agg}Read` je Aggregat + `process(): {BC}Process` + (Rules-Layer G10) optional ein direkter Methoden-Eintrag je **exponiertem rule-guarded Command** (früherer Begriff: „Haustür"). Kein allgemeiner Write-Accessor. |
...
| **Rule** | `{BC}/Rule/{Name}.php` — ein endpunkt-gebundener, synchroner Ja/Nein-Wächter (`__invoke({Cmd}DTO): RuleResult`), DeveloperOwned, BC-scoped, wiederverwendbar über Aggregate hinweg. Rufbar ausschließlich via `$this->handle({Rule}::class)` (ClassVersion-fähig, `Rule/v{N}/`) — nie `new`. Liest Bestand nur über die **eigene** BC-Lese-Fassade (M9-Verbot: kein Fremd-BC-Import). |
...
| Rule stub | `{BC}/Rule/<RuleName>.php` | DeveloperOwned (tag `RuleClass`) | `extends <Domain>Context`. `__invoke(<Cmd>DTO): RuleResult` — your bestand-check logic, reading only via `$this->handle({BC}::class)->{agg}()` (own-BC read facade, M9). Untouched bodies migrate wholesale on rebuild; a hand-edited body stays 100% verbatim (phpmerge). ClassVersion-fähig (`Rule/v{N}/<RuleName>.php`) — the **payload structure + messageKey** are the stable API across versions, only the accepted set may tighten (M5). |
...
| Query tree | `{Agg}/Query/{<Agg>ById,<Agg>ByIds,<Agg>By<Key>,<Agg>By<Custom>…,<Agg>ListFilter}.php` + `Query/Handler/Get<Agg>…Handler.php` (**WITH `Handler` suffix**) | ForceOverwrite | Read-base DTOs (ById/ByIds/By{UniqueKey}) + hand-modelled Source-query DTOs + handlers — no suffix-less `get<Agg>` exists |
...
| Repository tree | `{Agg}/Repository/{<Agg>Repository,Query<Agg>{ById,ByIds,By<Key>,…},Transform<Agg>,Validate<Agg>,Persist<Agg>,Query<Agg>{Additions,List}}.php` | ForceOverwrite | Read / write pipeline base; one root-fetch class per read variant (`WHERE pk =` / `pk IN` / `unique =` / hand-modelled); custom queries (generic CRUD = pipeline) |
...
`By<UniqueKey>` exists exactly once per aggregate — the identifier chain picks the unique business key (e.g. `getCounterByIdentifier(new QueryCounterByIdentifier(identifier: $i))`); an aggregate without a unique key gets only ById/ByIds. Hand-modelled Source queries add further `get<Agg>By<Custom>` methods (single-Akte collapse like today); `<agg>List(<Agg>ListFilter)` is the flat auto-list. A suffix-less `get<Agg>` does not exist. The ids are the **internal** handle of the domain API — the facade is no public REST surface; exposure is the surrounding app's decision — and FK/child internals stay encapsulated either way.
...
**Bulk-read recipe** — list items and the projected Akte both carry the root id, so the set pattern reads identically across all aggregates:
...
Edge behaviour (existing infrastructure semantics, nothing new underneath): empty `ids` → empty list · duplicates → deduped (IN semantics) · missing ids → partial result, no error (consistent with ById → `null`) · no order guarantee.
...
| Provide a **versioned variant** of a generated class (tenant / feature-flag) | Put the variant under `{Agg}/.../v{N}/<Class>.php` and activate via `version()` on the Domain facade. ClassVersion (`LoadClassFromSubDirectory`) resolves `…\v{N}\<Class>` before the baseline. **Note:** version *creation* is not wired by any generator today — the *resolution* is. See `platform-versioning`. |
| Guard a Command with a **business rule / bestand-check** before it runs (Rules-Layer) | Declare a catalog entry + endpoint binding in `Rules.yaml` (BC-level, sibling of Process/Steckbrief). Generator emits a Rule stub under `{BC}/Rule/{RuleName}.php` (`__invoke({Cmd}DTO): RuleResult`) plus a hermetic Guard closure that runs the bound chain (AND, short-circuit) ahead of the Command. Your logic lives in the Rule stub body — bestand-checks go through the **own-BC** read facade only (M9); anything needing a cross-BC read or a multi-step/side-effecting flow is Prozess-Territorium, not a Rule. |
...
| V13 | A Rule (`{BC}/Rule/{Name}.php`) reads bestand only via its **own BC's** read facade (`$this->handle({BC}::class)->{agg}()`) — no foreign-BC import, no cross-BC read from inside a Rule (M9). Enforced by convention + a stub docblock nudge, not yet by static analysis. A cross-BC bestand-check belongs in a Process, not a Rule. |
...
| Tenant- / feature-flag variant of any generated class | Put the variant under `{Agg}/.../v{N}/<Class>.php` and activate via `version()` on the Domain facade (see `platform-versioning`). Resolution is wired; creation is out of current scope. | ClassVersion |
...
| 2a | Endpoint business rule (bestand-check gate before a Command runs) | `Rules.yaml` catalog + binding → `{BC}/Rule/{Name}.php` stub, your logic in `__invoke()` (own-BC read facade only, M9) | — (Rules-Layer, no separate package skill) |
...
- `DomainResponse`, `ContextResponse`, `DomainResponseTransformer`: generated per domain (`{Domain}\Response\`, Response-Trio, §7 above) — no longer package classes; `ResponseStatus` + the response/context interfaces: `jardissupport/contract`
...
- `docs/rules-layer/PLAN.md` (Builder repo) — Rules-Layer implementation detail (contracts E1–E3, Verträge 1–5, phase-by-phase build log); `docs/rules-layer/PRD.md` for the fachliche Begründung

...
├── Api/
│   └── FieldMap.php                              ← central FieldMap (Generated, ForceOverwrite)
├── Config/
│   ├── .env
...
└── <BC>/
    ...
    └── <Agg>/                                    ← Dev-owned root (parallel to Platform/)
        ...
        ├── Api/                                  ← Dev-API surface (registry overrides)
...
| Dev-Code root | `<BC>/<Agg>/` (parallel to `Platform/`) | Developer-owned | VOs, services, registry overrides, new commands/queries — only place the developer writes |

**Two file modes from the generator:**

- **ForceOverwrite** — rewritten on every build. All Platform/ files plus Domain/BC facades.
...
**No generated OpenAPI / AsyncAPI / Proto specs.** The builder used to emit them but the generator was removed in 2026-04-26 (`f46195b6`) — those specs documented interfaces (HTTP server, message broker, gRPC server) that don't exist in the generated code. External / fachliche API will arrive later as a dedicated developer-owned generator path. Today the only thing the `<Agg>/Api/` and `<Domain>/Api/` directories contain are runtime registries (`<Agg>{Commands,Queries,Events}.php`) and the central `FieldMap.php`.
...
├── Api/<Agg>Commands.php                   ← registry override (extends Platform\Api\<Agg>Commands)

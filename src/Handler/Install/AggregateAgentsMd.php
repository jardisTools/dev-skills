...
final class AggregateAgentsMd
{
    ...
    /** @var Closure(list<AgentsDescriptor>, bool): string */
     ...
     *
     * - Empty descriptor list AND catalog not installed → no file is written, result has count 0.
     * - Empty descriptor list AND catalog installed → managed block with pointer only is written.
    ...
    public function __invoke(
        ...
        if ($descriptors === [] && !$catalogInstalled) {
        ...
        $block = ($this->buildBlock)($descriptors, $catalogInstalled);
        ...
        return new AggregateAgentsResult(count($descriptors), $backupPath, $analysis->healedDuplicateBlock);
    }

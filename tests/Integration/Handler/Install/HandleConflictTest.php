...
final class HandleConflictTest extends TestCase
{
    ...
    public function testDoesNotCreateSecondBackupStageForBackupDirectory(): void
    {
        $this->project->writeFile('.claude/skills/adapter-cache.backup/SKILL.md', 'stale');
        $target = $this->project->path('.claude/skills/adapter-cache.backup');

        $result = (new HandleConflict(new Filesystem()))($target);

        self::assertSame($target, $result);
        self::assertDirectoryDoesNotExist($target);
        self::assertDirectoryDoesNotExist($target . '.backup');
    }

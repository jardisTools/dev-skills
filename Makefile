...
generate-catalog-check: ## Check that the checked-in SKILL.md matches the manifest (exit≠0 on drift)
	$(DOCKER_COMPOSE) run --rm --no-deps phpcli php /app/bin/generate-catalog.php --check
.PHONY: generate-catalog-check

check-catalog-packagist: ## Check Packagist for Jardis packages missing from catalog/manifest.json (warning only, exit 0)
